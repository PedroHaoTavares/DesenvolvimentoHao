...
/**
 * ✅ NOVO: Enviar Localização (latitude/longitude)
 * Rota: POST /messages/sendLocation/:ticketId
 * Body: { latitude, longitude, isPrivate? }
 */
export const sendLocation = async (
  ...
  const { latitude, longitude, isPrivate = "false" } = req.body as {
  ...
  try {
    ...
    // Se for privado, salva somente no banco (não envia pro WhatsApp)
    ...
    // Só implementamos envio real via WhatsApp aqui (como no seu frontend)
    ...
    // Salva no banco para aparecer no chat do sistema
    const messageBody = `📍 Localização\nLat: ${latNum}\nLng: ${lngNum}`;
    ...
    return res.status(200).json({ message: "Localização enviada com sucesso", sent });
  } catch (error: any) {
...
export const forwardMessage = async (
  ...
  // (mantido igual ao que você enviou)
